...
export default function App() {
  ...
  const addTodo = async (data) => {
    ...
    if (data.id != "") {
      let filter = todos;
      let objectId = filter.findIndex((obj) => obj.id == data.id);
      filter[objectId].title = data.title;
      filter[objectId].description = data.description;
      storeTodos(filter);
    } else {
      const newTodo = [
      ...
      <View style={styles.container}>
        ...
        {showAdd.show && (
          <Animatable.View style={styles.modal} animation="fadeInUpBig">
            <Modal show={showAdd} addTodo={addTodo} setAdd={setAdd} />
          </Animatable.View>
        )}
      </View>
